...
| **LLM layer** | `LLMProvider` ABC. `GeminiProvider` (default, `gemini-3.5-flash`, `google-genai`) + `AnthropicProvider` + `FakeProvider` (deterministic, offline, for CI/dev/demo). Pydantic→Gemini schema converter for function calling. Token & cost tracking, retries, timeouts. |
...
| **Embeddings** | `EmbeddingProvider` ABC — `gemini` (`gemini-embedding-001`, 768-dim, default), `fastembed` (local), `voyage` (hosted), `hash` (deterministic, offline). |
