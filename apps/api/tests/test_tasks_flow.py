...
async def _dev_repo() -> str:
    async with AsyncSessionLocal() as db:
        ...
        if user is None:
            ...
            await db.flush()
        repo = Repository(
            user_id=user.id, github_repo_id=1, full_name="dev/app",
            default_branch="main", private=True, clone_url="https://x/app.git",
        )
        db.add(repo)
        await db.flush()
        db.add(RepositoryVersion(
            repository_id=repo.id, branch="main", commit_sha="a" * 40, status=IndexStatus.READY,
        ))
        await db.commit()
        return str(repo.id)
...
async def test_create_task_rejects_short_description(client):
    repo_id = await _dev_repo()
    r = await client.post("/api/tasks", json={
        "repository_id": repo_id, "branch": "main", "title": "x too short", "description": "no",
    })
...
async def test_run_task_enqueues(monkeypatch, client):
    ...
    created = await client.post("/api/tasks", json={
        ...
        "title": "Fix the checkout bug", "description": "Investigate and fix the 500 on checkout.",
    })
...
async def test_diff_endpoint_empty_before_run(client):
    ...
    created = await client.post("/api/tasks", json={
        "repository_id": repo_id, "branch": "main",
        "title": "Some engineering task", "description": "Do the thing described here.",
    })
    task_id = created.json()["id"]
